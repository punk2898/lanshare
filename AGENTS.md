...
1. 在仓库根目录后台启动，并把输出写到日志里（这个服务要一直运行，不能放在前台等它结束）：
   ...
   ```bash
   mkdir -p ~/LANShare && nohup ./scripts/start.sh > ~/LANShare/lanshare.log 2>&1 &
   ```

   Windows 上没有 `start.sh`：有 Go 就执行 `go run ./cmd/lanshare`（放后台运行）；没有 Go 就从本仓库的 GitHub Releases 下载 `lanshare-windows-amd64.exe` 来运行。

2. 等两三秒后读日志 `cat ~/LANShare/lanshare.log`，把里面的「其他设备」地址和「访问码」告诉用户。
...
- `cmd/lanshare/`：启动程序（命令行 / Mac App 共用）
